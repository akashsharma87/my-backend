...
export default function EngineerAuth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        await signIn(email, password, 'engineer');
      } else {
        await signUp(email, password, fullName, 'engineer');
      }
        ...
        <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="w-16 h-16 bg-mono-1000 rounded-sm flex items-center justify-center mb-6 mx-auto lg:mx-0">
              <Code className="h-8 w-8 text-mono-0" />
            </div>
            ...
            <p className="text-lg text-mono-600 mb-8 leading-relaxed">
              {isLogin ? 'Welcome back to your engineering dashboard' : 'Join thousands of engineers building their careers'}
            </p>
              ...
              {[
                {
                  ...
                  description: "Upload your resume and highlight your technical expertise with a clean, professional profile."
                },
                {
                  title: "Smart Matching",
                  description: "Our algorithm connects you with opportunities that match your skills and career goals."
                },
                {
                  title: "Direct Connect",
                  description: "Skip the noise. Connect directly with hiring managers and technical teams."
                }
              ].map((benefit, index) => (
                <motion.div
                  key={index}
          ...
          >
            <div className="card-minimal p-10">
              <div className="text-center mb-10">
                <div className="w-12 h-12 bg-mono-1000 rounded-sm flex items-center justify-center mx-auto mb-6">
                  <User className="h-6 w-6 text-mono-0" />
                </div>
                <h2 className="text-2xl md:text-3xl font-light text-mono-1000 mb-3">
                  {isLogin ? 'Welcome Back' : 'Join Engineer.CV'}
                </h2>
                <p className="text-mono-600">
                  {isLogin ? 'Access your engineering profile' : 'Start your engineering journey'}
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                {!isLogin && (
                  <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                  >
                    <label className="form-label text-mono-800">Full Name</label>
                    <div className="relative">
                      <User className="absolute left-4 top-1/2 transform -translate-y-1/2 h-4 w-4 text-mono-500" />
                      <input
                        type="text"
                        value={fullName}
              ...
              <div className="space-y-6 mt-8">
                ...
                <div className="text-center pt-6 border-t border-mono-200">
                  <p className="text-mono-600 mb-3 text-sm">Looking to hire engineers?</p>
